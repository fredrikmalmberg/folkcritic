...
config = importlib.import_module('configurations.config5')
# samples dir
if not os.path.isdir('samples'):
        os.makedirs('samples')
target_path = "samples/%s-s%d-%.2f-%s.txt" % (
    metadata[b'experiment_id'], rng_seed, temperature, time.strftime("%Y%m%d-%H%M%S", time.localtime()))

token2idx = metadata[b'token2idx']
print('token2idx is:', token2idx)
...
lasagne.layers.set_all_param_values(l_out, metadata[b'param_values'])
