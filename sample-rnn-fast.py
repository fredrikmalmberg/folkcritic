...
if seed is not None:
    ...
    for tok in seed_sequence[:-1]:
        ...
        for jj in range(numlayers):
            ...
            ct = np.multiply(ft, ctm1[jj]) + np.multiply(it, np.tanh(
                np.dot(x, LSTM_Wxc[jj]) + np.dot(htm1[jj], LSTM_Whc[jj]) + LSTM_bc[jj]))
...
with open('folk-rnn-master/vocabulary.txt', 'w') as outfile:
